  ...
  <div class="p-4 md:p-6 lg:p-8 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
      ...
      <button
        ...
        class="flex items-center gap-2 px-4 py-2 mb-6 text-gray-700 bg-white shadow-sm hover:bg-blue-50 rounded-full transition"
      >
      ...
      >
        <div>
          ...
          <p class="text-gray-500 mt-1 font-medium">
            Overview of outstanding customer balances.
          </p>
        </div>
        <Button
          label="Print Report"
      ...
      >
        ...
        <DataTable
          ...
          emptyMessage="No outstanding debts found for the applied filters."
        >
          ...
          <Column field="updatedAt" header="Last Updated" sortable>
            <template #body="slotProps">
              <span class="text-gray-600">
                {{
                  formatDate(
                    slotProps.data.updatedAt || slotProps.data.createdAt,
                  )
                }}
              </span>
            </template>
          ...
          <Column field="invoiceIds.length" header="Unpaid Invoices" sortable>
            <template #body="slotProps">
              <span
                class="bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-sm font-bold"
              >
                {{ slotProps.data.invoiceIds?.length || 0 }} Invoices
              </span>
            </template>
          </Column>
          ...
          >
            <template #body="slotProps">
              <div class="font-black text-red-600 text-lg text-right">
                {{ formatCurrency(slotProps.data.totalDebt) }}
              </div>
            </template>
          </Column>
        </DataTable>
...
import DatePicker from "primevue/datepicker"; // Changed to DatePicker as requested
...
const fetchReportData = async () => {
  ...
  try {
    const [custRes, reimRes] = await Promise.all([
      getDocs("Customer"),
      getDocs("CustomerReimburstment"),
    ]);

    customers.value = custRes?.data || [];
    const reimbursements = reimRes?.data || [];

    // Base Filter: Only records for the current branch with active debt > 0
    const currentBranch = branchStore.branchId || "";
    const activeDebts = reimbursements.filter((r) => {
      const isCorrectBranch = (r.branchId || "") === currentBranch;
      const hasDebt = Number(r.totalDebt) > 0;
      return isCorrectBranch && hasDebt;
    });

    // Map customer names into the records for easy display and sorting
    debtors.value = activeDebts.map((debt) => {
      const custId =
    ...
    // Sort by largest debt first globally
    debtors.value.sort((a, b) => Number(b.totalDebt) - Number(a.totalDebt));

    // Apply default filters immediately on load
    applyFilters();
  } catch (error) {
    console.error("Failed to fetch debt report data:", error);
  } finally {
...
const filteredDebtors = computed(() => {
  return debtors.value.filter((d) => {
    ...
    if (
      ...
      d.extractedCustomerId !== appliedFilters.value.customer
    ) {
      return false;
    }
...
const printReport = () => {
  ...
  // Calculate dynamically filtered total debt
  const totalFilteredDebt = filteredDebtors.value.reduce(
    (sum, item) => sum + (Number(item.totalDebt) || 0),
    0,
  );

  let rowsHtml = filteredDebtors.value
    .map(
      (d, index) => `
    <tr>
      <td>${index + 1}</td>
      <td style="text-align: left;">${d.customerName}</td>
      <td>${d.invoiceIds?.length || 0}</td>
      <td>${formatDate(d.updatedAt || d.createdAt)}</td>
      <td style="text-align: right; color: #cc0000;">${formatCurrency(
        d.totalDebt,
      )}</td>
    </tr>
  `,
  ...
  if (filteredDebtors.value.length === 0) {
    rowsHtml = `<tr><td colspan="5">No outstanding debts found for the applied filters.</td></tr>`;
  }
  ...
  const printContent = `
    ...
    <head>
      ...
      <style>
        ...
        .summary-box .amount { margin: 0; font-size: 24px; font-weight: bold; color: #cc0000; }
    ...
    <body>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Last Updated</th>
            <th style="text-align: right;">Total Debt Amount</th>
          </tr>
        </thead>
        <tbody>
          ${rowsHtml}
      ...
      <div class="clearfix">
        <div class="summary-box">
          <h3>Total Filtered Debt</h3>
          <p class="amount">${formatCurrency(totalFilteredDebt)}</p>
        </div>
      </div>
