<template>
  <div class="mx-5 font-noto">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 my-4 border-2 border-dashed border-[#5B9717]">
      <h1 class="text-2xl md:text-3xl font-bold text-[#045B1B] mb-6">Lottery Chief Balances</h1>
      ...
      <div class="flex flex-wrap items-end justify-between gap-4">
        <div class="flex gap-4 items-end flex-1">
          ...
          <div class="w-full md:w-96">
            ...
            <input v-model="searchQuery" type="text" placeholder="Search by Invoice ID or Expense..."
      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-[#045B1B] text-white">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-center">Invoices</th>
...
<td class="px-6 py-4 whitespace-nowrap font-bold text-[#045B1B]">
  {{ item.amount?.toLocaleString('en-US', { maximumFractionDigits: 0 }) }}
            ...
            <td class="px-6 py-4">
              <i class="pi" :class="item.status ? 'pi-check-circle text-green-600' : 'pi-times-circle text-red-600'" />
            </td>
            <td class="px-6 py-4 text-center text-sm font-medium">
              <button @click="openEditForm(item)" class="text-blue-600 hover:bg-blue-50 p-2 rounded-md mx-1">
          ...
          <tr v-if="tableData.length === 0 && !isLoading">
            <td colspan="5" class="px-6 py-10 text-center text-gray-500 italic">No balance data found.</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-else>
      <LotteryChiefBalanceCard :items="tableData" :is-loading="isLoading" @onEdit="openEditForm" @onDelete="confirmDelete" />
    </div>

    <div class="mt-5">
      <Pagination :currentPage="currentPage" :limitedPerPage="pageSize" :searchQuery="searchQuery"
        collectionName="LotteryChiefBalance" @onEmitDataFromPagination="tableData = $event"
        @onEmitIsLoading="isLoading = $event" />
    </div>

    <LotteryChiefBalanceForm :visible="showFormModal" :is-edit-doc="isEditDoc" :doc="selectedDoc" @onClose="closeForm" />
    
    <DeleteConfirmation :visible="showDeleteModal" :deleteId="deleteId" :elementName="'Balance Record'"
      collectionName="LotteryChiefBalance" @onCloseDelete="showDeleteModal = false" />
  </div>
</template>
...
export default {
  components: { ChevronLeftIcon, PlusIcon, Pagination, DeleteConfirmation, LotteryChiefBalanceForm, LotteryChiefBalanceCard },
  setup() {
    ...
    const closeForm = (status) => { showFormModal.value = false; if (status === 'add' || status === 'update') searchQuery.value = ''; };
