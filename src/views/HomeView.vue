    ...
    <div class="w-full max-w-screen-xl relative px-2 sm:px-4">
      ...
      <div @click="nav = true"
        ...
        <div class="text-left flex-1 min-w-0 pr-4">
          ...
          <p class="text-base sm:text-xl font-medium text-gray-600 mt-1">{{ t('Branch') }}: {{ user.branch }}</p>
        </div>
          ...
          <div class="fixed inset-x-0 bottom-0 bg-white rounded-t-3xl shadow-2xl max-h-[90vh] overflow-y-auto" @click.stop>
            <div class="h-48 bg-[rgb(var(--primary-rgb))] rounded-t-3xl relative">
              <div class="absolute top-4 left-4 right-4 flex justify-between items-center">
                ...
                <h2 class="text-xl font-semibold text-white">{{ t('Settings') }}</h2>
              </div>
            ...
            <div class="pt-24 pb-8 px-4">
              <div class="space-y-3">
                ...
                <div @click="toggleLanguage"
                  ...
                  <div class="flex items-center gap-4">
                    <div class="w-6 h-6 flex items-center justify-center font-bold text-[rgb(var(--primary-rgb))]">
                      {{ locale === 'kh' ? 'EN' : 'KH' }}
                    </div>
                  ...
                  <span class="text-xs font-bold text-gray-400 uppercase">{{ locale }}</span>
                </div>
              </div>
              ...
              <div @click="showThemes = !showThemes"
                ...
                <div class="flex items-center gap-4">
                  ...
                  <span class="text-lg font-semibold text-gray-700">{{ t('themes') }}</span>
                </div>
              ...
              <transition name="fade">
                <div v-if="showThemes" class="mt-4 p-4 bg-gray-100 rounded-xl">
                  <div class="grid grid-cols-5 gap-3">
                    <button v-for="color in ['blue', 'purple', 'green', 'red', 'orange']" :key="color" 
                      @click="setTheme('theme-' + color)"
                      :class="['w-10 h-10 rounded-xl transition hover:scale-105 ring-4', `bg-${color}-500 ring-${color}-500/50`]"></button>
                  </div>
                </div>
              </transition>
      ...
      <div v-if="isRootPath" class="w-full">
          ...
          <div class="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
            <div class="flex justify-around text-center divide-x divide-gray-200">
              <div class="pr-4 flex-1">
                <h3 class="text-lg font-semibold text-gray-500">{{ t('Income') }}</h3>
                <p class="text-xl sm:text-2xl font-extrabold text-green-500 mt-1">${{ totalIncome }}</p>
              </div>
              <div class="pl-4 flex-1">
                <h3 class="text-lg font-semibold text-gray-500">{{ t('Expense') }}</h3>
        ...
        <div class="mt-12 px-2 sm:px-4">
          <!-- <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-6 max-w-2xl mx-auto">{{ t('quickActions') }}</h2> -->
          <div class="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
            <router-link v-for="item in gridMenus" :key="item.label" :to="item.to" class="text-center group relative">
              <div class="bg-white p-4 sm:p-6 rounded-3xl shadow-lg border-2 border-transparent hover:border-[rgb(var(--primary-rgb))] hover:bg-[rgb(var(--primary-rgb))/0.1] transition-all duration-300 transform hover:scale-[1.03]">
                ...
                <p class="font-bold text-sm sm:text-base text-gray-700 group-hover:text-[rgb(var(--primary-rgb))]">{{ item.label }}</p>
              </div>
            </router-link>
          </div>
        </div>
...
const user = ref({ name: 'Loading...', branch: '...', imageURL: defaultProfile })
...
{ label: t('Customers'), to: '/customer', icon: UsersIcon },
  ...
  { label: t('Reimbursement'), to: '/customer-reimburstment', icon: BanknotesIcon },
  { label: t('Return Money'), to: '/customer-return-money', icon: ArrowUturnLeftIcon },
...
const fetchUserData = async () => {
  try {
    ...
    const conditions = JSON.stringify([{ field: '_id', operator: '==', value: userId }]);
    const res = await axios.get(`${API_CONFIGS.BASE_URL}/api/getAllDocs/User`, { params: { dynamicConditions: conditions } });
    const userData = res.data.data?.[0];
    if (userData) {
      user.value = {
        name: userData.username,
        branch: userData.mainRole === 'Super Admin' ? 'System Admin' : (branchStore.branchName || 'Branch'),
...
onMounted(() => {
  document.documentElement.classList.add(localStorage.getItem('app-theme') || 'theme-blue')
...
const handleMenuClick = (path) => {
  nav.value = false
  if (path === '/logout') {
    localStorage.clear(); branchStore.logoutClear(); router.push('/login');
...
.slide-up-enter-active, .slide-up-leave-active { transition: transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
