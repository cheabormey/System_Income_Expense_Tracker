      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-[#045B1B] text-white">
          <tr>
            ...
            <th
              class="px-6 py-3 text-left text-xs font-medium uppercase text-center"
            >
              Status
            </th>
        ...
        <tbody class="divide-y divide-gray-200">
          <tr
            ...
            :key="item._id"
            :class="index % 2 === 0 ? 'bg-white' : 'bg-[#f9fafb]'"
          >
            <td class="px-6 py-4">{{ formatDate(item.returnDate) }}</td>
            <td class="px-6 py-4 font-medium">
              {{ formatNameById(item.customerId, customers) }}
            </td>
            <td class="px-6 py-4 text-[#5B9717] font-bold">
              {{ formatKHR(item.amount) }}
            </td>
            ...
            <td class="px-6 py-4 text-center space-x-2">
              <button
                ...
                class="text-blue-600 hover:bg-blue-50 p-2 rounded-md"
              >
                <i class="pi pi-pencil" />
              </button>
              <button
                @click="confirmDelete(item)"
                class="text-red-600 hover:bg-red-50 p-2 rounded-md"
              >
                <i class="pi pi-trash" />
              </button>
            </td>
...
import { formatKHR } from "@/helper/formatKHR";
